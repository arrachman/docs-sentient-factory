...
```markdown
> **Parent Doc:** [BPA-1](bpa-1.md), [System Analysis](system_analysis.md)
...
### Documents to Update:
- [ ] bpa-1.md - Section 2.1 Login
- [ ] system_analysis.md - Section 1.7 Admin Module
- [ ] ui-design.md - Section 4.1 Login Page
```
...
- **Gate 1**: Requirements → Analysis (Stakeholders sign-off, Scope approved)
- **Gate 2**: Analysis → Design (BPA approved, ERD finalized, NFR documented)
- **Gate 3**: Design → Development (Architecture approved, UI/UX approved)
- **Gate 4**: Development → Testing (Code complete, Unit tests >70%, Code review done)
- **Gate 5**: Testing → Deployment (All tests passed, UAT signed, Security scan passed)
- **Gate 6**: Deployment → Go-Live (Smoke test passed, Monitoring configured)

### 8.4 Roles & Responsibilities (RACI)

**Legend:** R = Responsible, A = Accountable, C = Consulted, I = Informed

- **Requirements**: PO (R), Architect (C)
- **Analysis**: Analyst (R), Dev (C)
- **Development**: Dev (R), QA (C)
- **Testing**: QA (R), Dev (C)
- **Deployment**: DevOps (R), QA (C)
...
- `bugfix/SF-XXX-description`
- `hotfix/SF-XXX-description`

**Commit Format:** `[SF-XXX] type: description`
Types: feat, fix, docs, style, refactor, test, chore

### 8.7 Sprint Cycle (2 weeks)

- Day 1: Sprint Planning
- Day 2-9: Development
- Day 5: Mid-Sprint Review
- Day 10: Code Freeze, Testing, Review, Retro
...
| Minor    | 24 hours  | 1 week      |
| Low      | Scheduled | Next sprint |

---

## 9. Vibe Coding Rules (AI Assistant Compliance)

> **CRITICAL:** Semua AI assistant WAJIB mengikuti rules ini saat generate code.
> Full documentation: [docs/vibe-coding.md](docs/vibe-coding.md)
...
| **Permission Check** | Selalu add guards dari security.md |

### 9.3 Response Format (WAJIB)
...
```
// Frontend
[Module][Entity][Action].tsx       // SalesOrderListPage.tsx
use[Entity][Action].ts             // useSalesOrders.ts
...
```typescript
// ❌ WRONG
interface MyCustomType { }         // Don't create new types
const TAX = 0.11;                  // Don't hardcode values
return { id: 123 };                // Don't expose internal IDs
return { status: 'ok' };           // Don't use custom format

// ✅ CORRECT
import { SalesOrder } from '@/types/sales-order';  // From docs
const taxRate = config.get('TAX_RATE');            // From config
return { uuid: entity.uuid };                       // UUID only
return { success: true, data };                    // Standard format
```

### 9.6 data-testid Convention

```html
data-testid="btn-[action]"     // btn-create, btn-submit
data-testid="input-[field]"    // input-username
data-testid="[entity]-row"     // so-row
data-testid="menu-[module]"    // menu-sales
```

### 9.7 Quick Documentation Reference

| Topic | Document |
|-------|----------|
| API Schemas | docs/api-contract.md |
| Frontend | docs/frontend.md |
| Backend | docs/backend.md |
| Database | docs/database.md |
| AI Service | docs/python-ai.md |
| Tests | docs/qa.md |
| Security | docs/security.md |
| Deploy | docs/deployment.md |

**⚠️ AI: SELALU konsultasi dokumentasi sebelum generate code baru!**
